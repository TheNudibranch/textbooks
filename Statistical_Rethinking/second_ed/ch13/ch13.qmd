...
We can no longer rely soley on summary tables of means to understand the model. We need to understand how the model behaves in light on new data - counterfactuals. 

### 13.1 Posterior Predictions for the same cluster

Let's look at the chimp data again, for that there are 7 actors. Let's simulate predictions for actor 2:
...
We could have course done this without the `link` function:
...
p_raw <- vapply(1:4,  p_link, actor=2, block_id=1, FUN.VALUE=numeric(nrow(post$a)))
p_mu <- colMeans(p_raw)
p_ci <- p_raw |> apply(2, PI)
```

### 13.2 Posterior Predictions from new clusters

How we would go about getting the model accuracy for actor 7, if it was never included in the model. We might want to just use $\bar \alpha$, but as we'll see, that uncertainty is much smaller than what we would actually want it to be. 
...
Notice that we don't include `block` since we are generalizing to a new blocks, and we assumed that the average block effect is zero.

```{r}
p_raw <- vapply(1:4, p_link_abar, FUN.VALUE=numeric(nrow(post$a_bar)))
p_mu <- colMeans(p_raw)
p_ci <- p_raw |> apply(2, PI)
...
This is understated though, to see the actual variation among actors we need to simulate $\alpha \sim \text{N}(\bar \alpha, \sigma_{\alpha})$:


```{r}
a_sim <- with(post, rnorm(length(a_bar), a_bar, sigma_a))
...
So which should we use? It depends. If we want to see the impact of the treatment, we should probably do the first one. If we want to understand the spectrum of possible actors, the second one. But, the second plot has all possible actors placed together - those actors should still all be influenced by the treatment. That is, we should see the zig-zag line.


```{r}
plot(NULL, xlab='treatment', ylab='Proportion pulled left', ylim=c(0,1), xaxt='n', xlim=c(1,4))
axis(1, at=1:4, labels=c('R/N', 'L/N', 'R/P', 'L/P'))
for (i in 1:50) lines(1:4, p_raw[i,], col=grau(0.25), lwd=2)
```

### 13.3

Mister P - multilevel poststratification is a popular framework for reweighting survey response to a national or population. Basically, we have a survey $R$ where we want to measure peoples voting intentions $V$. This might be influenced by things like age $A$. The survey $R$ probably has a different age distribution than the total population so we have to reweight accoridingly. This becomes hairy when $R \leftarrow A \rightarrow R$, but becomes downright impossible when $V \rightarrow R$ as well. Since we won't be able to isolate the effect of $A$ on $V$, which is required to generalize to the total population.
...
We'll now change to slightly to have:
...
Essentially, we are just adding a mean term for the adaptive prior for blocks.
...
I'll leave the full output out, but it looks like we got a lot of divergences when we fit. This is of course becuase the model wants one simple mean that it can perturb. We are back to the left/right leg problem again.

```{r}
extract.samples(m_h5)[c('g_bar', 'a_bar')] |> as.data.frame() |> cor()
```
...
Looks like it is not significantly better.

### M6

Let's try four different models for the point $y=0$. We'll see the effects of different tails on the mean.
...
Because the rethinking package is throwing an error with this version of cmdstanar, we'll instead use `c(0,0)` for the data.
...
nn <- ulam(alist(
  y ~ dnorm(mu, 1),
  mu ~ dnorm(10, 1)
), data=list(y = c(0,0)) , cores=4, chains=4, log_lik=T)
...
Looks like the ones that impose a $t$ prior on $\mu$ perform the best. This makes sense as we want $\mu$ to be flexible and able to move with high outlier data for this example. That is, if $\mu$ has a prior with long tails, the likelihood can pull it in. 


Now we'll plot all the posteriors:

```{r}
# 1. Extract posterior samples for mu from each model
post_nn <- extract.samples(nn)$mu
post_tn <- extract.samples(tn)$mu
post_nt <- extract.samples(nt)$mu
post_tt <- extract.samples(tt)$mu

# 2. Calculate densities
d_nn <- density(post_nn)
d_tn <- density(post_tn)
d_nt <- density(post_nt)
d_tt <- density(post_tt)

# 3. Setup the plot range
xlim <- range(c(post_nn, post_tn, post_nt, post_tt))
ylim <- range(c(d_nn$y, d_tn$y, d_nt$y, d_tt$y))

# 4. Create the base plot
plot(d_nn, xlab = expression(mu), main = "Posterior Distributions of mu",
     xlim = xlim, ylim = ylim, lwd = 2, col = "black")

# 5. Add the other three models
lines(d_tn, lwd = 2, col = "blue")
lines(d_nt, lwd = 2, col = "red")
lines(d_tt, lwd = 2, col = "darkgreen")

# 6. Add a legend
legend("topright", 
       legend = c("Normal Lik / Normal Prior", 
                  "Student Lik / Normal Prior", 
                  "Normal Lik / Student Prior", 
                  "Student Lik / Student Prior"),
       col = c("black", "blue", "red", "darkgreen"), 
       lwd = 2)

# Optional: Add a vertical line at the data mean (0) and prior mean (10)
abline(v = 0, lty = 2, col = "gray")
abline(v = 10, lty = 2, col = "gray")
```
Looks like the student-t priors for $\mu$ are almost on top of each other. What's interesting is that the TN model moves the least from 10. This is because the liklihood is at its weakest here.

### H2

Let's return to the Trolley problem from chaper 12. This time we'll fit a varying intercept model:
...
# ), data=dat, chains=4, cores=4, iter=400)
```

With the addition of the varying intercept:
...
# m_h2 <- ulam(alist(
#   R ~ dordlogit(phi, cutpoints),
#   phi <- bA*A + bC*C + bI*I + b_ID[id],
#   c(bA, bC, bI) ~ dnorm(0,0.5),
#   b_ID[id] ~ dnorm(0, sig_ID),
#   sig_ID ~ dexp(1),
#   cutpoints ~ dnorm(0,1.5)
# ), data=dat, chains=4, cores=4, iter=400)
```
