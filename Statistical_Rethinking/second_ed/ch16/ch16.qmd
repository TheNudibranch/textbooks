---
title: "Rethinking - Chapter 16"
format:
  html:
    code-fold: show
toc: true
---
```{r}
#| output: false
#| code-fold: true

library(rethinking)
library(dagitty)
library(tidyverse)
library(cmdstanr)
# rstudioapi::getActiveDocumentContext()$path |>
#   dirname() |>
#   setwd()
```
## 16.0

Time to move beyond the limits of simple GLM models and build something that is actually *tailored* to the problem at hand.

## 16.1 - Geometric People

Let's model the height and weight of persons assuming that people take similar shape of a cylinder: $V = \pi r^2 h$. We'll let $r$ be some proportion of the height such that $r=ph$. Thus, $V = \pi p^2 h^3$.

$$
\begin{align}
W_i &\sim \operatorname{LogNormal}(\mu_i, \sigma) \\
\exp(\mu_i) &= k\pi p^2 h_i^3 \\
k &\sim \text{some prior}  \\
p &\sim \text{some prior}  \\
\sigma &\sim \operatorname{Exponential}(1)
\end{align}
$$

The mean of the lognormal depends on $\sigma$, so in order to avoid causing issues, we'll set the median of the distribution at the theoretical average predicted by the cylindrical formula.

Notice that $k$ and $p$ are unidentified since they multiply together and therefore are unable to pin down separately. To fix this we can define $\theta = p k$. But we will still need to think about the priors.

Now let's think about the priors. For $p$, it is clear that it must be positive so we don't have negative width. It should also be less than 1 since people are thinner than they are tall. Finally, we can make the case that $p$ should be less than 0.5 since that is empirically what we see in the data. A good prior would be $p \sim \text{Beta}(2,18)$ which puts the average around 0.1.

Now onto $k$. If we scale our data:


```{r}
data(Howell1)
d <- Howell1
d$w <- d$weight / mean(d$weight)
d$h <- d$height / mean(d$height)
```

Then for the average: $1 = k \pi p^2 1^3$ and for $p < 0.5$, we know $k$ must be greater than 1. Thus, we'll use $k \sim \text{Exponential}(0.5)$.

```{r}
stan_data <- list(
  N = nrow(d),
  w = d$w,
  h = d$h
)

m16.1 <- cmdstan_model("mod_16.1.stan")
fit16.1 <- m16.1$sample(
  data = stan_data,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 250, iter_sampling = 300,
  show_messages = F, show_exceptions = F
)
```

```{r}
#| output: asis
#| echo: false
stan_source <- htmltools::htmlEscape(
  paste(readLines("mod_16.1.stan"), collapse = "\n")
)
cat(
  "<details>\n",
  "<summary>Stan code for model 16.1</summary>\n",
  "<pre><code>", stan_source, "</code></pre>\n",
  "</details>\n",
  sep = ""
)
```

Let's take a look at the posteriors:

If you look at the ridge, that is where the two parameters $k$ and $p$ produce the same product $kp^2$. We also see a strong negative correlation since as one increases, the other must decrease to compensate. But this happens *quadratically* since we have $p^2$ defining $\theta$.

```{r}
fit16.1$draws(c("p", "k"), format = "df") |>
  select(-starts_with(".")) |>
  sample_n(1e3) |>
  bayesplot::mcmc_pairs()
```

```{r}
h_seq <- seq(from = 0, to = max(d$h), length.out = 30)
posterior <- fit16.1$draws(c("p", "k", "sigma"), format = "df")
w_sim <- vapply(
  h_seq,
  function(h) {
    rlnorm(
      nrow(posterior),
      meanlog = log(3.141593 * posterior$k * posterior$p^2 * h^3),
      sdlog = posterior$sigma
    )
  },
  numeric(nrow(posterior))
)
mu_mean <- apply(w_sim, 2, mean)
w_CI <- apply(w_sim, 2, PI)

plot(
  d$h,
  d$w,
  xlim = c(0, max(d$h)),
  col = rangi2,
  lwd = 2,
  xlab = "height (scaled)",
  ylab = "weight (scaled)"
)
lines(h_seq, mu_mean)
shade(w_CI, h_seq)
```

It's worth pointing out that we could have easily taken the log of both sides and this would have become a simple GLM:

$$
\begin{align}
\log(w_i) &= \log(k \pi p^2h_i^3) \\
&= \log(k) + \log (\pi) + 2\log(p) + 3\log(h_i)
\end{align}
$$

Where the intercept absorbs all the terms we were modelling and there is a fixed '3' coefficients on $\log h_i$.

## 16.2 - Hidden minds and observed behavior

Consider the `Boxes` dataset. In it there are 629 children tested. In the experiment, the observer (child) watches 4 demonstrators. The first 3 demonstrators choose the same colored box. The next demonstrator chooses a different color from the first three. Finally, the child is asked to choose a color. The boxes don't do anything, we are simply looking to see if there is any correlation between what the demonstrators chose, and what the child chose.


```{r}
data(Boxes)
d <- Boxes
precis(d)
```

The `y` outcome here is 1 if the unchosen color (by the demonstrators) was chosen, 2 if the majority color was chosen, and 3 if the minority color was chosen. Another variable of interest is `majority_first`. This tells us if the majority color was demonstrated first before the minority color.


```{r}
table(d$y) / length(d$y)
```

Looks like the majority color was the most popular across trials.

The goal here is to infer strategy, not frequencies. That is why we will not be using a GLM. To do this, we will need to define some possible strategies:

1. Follow the majority
2. Follow the minority
3. Maverick: Choose the color that no demonstrator chose
4. Random
5. Follow first: copy the first color that was demonstrated

All three of these strategies imply some vector of three probabilities. Random would be a flat 1/3 for each outcome.

To do inference on this, we'll need to marginalize out the strategy $s$: $\Pr(y_i) = \sum_{s=1}^{5} p_s \Pr(j \mid s)$

$$
\begin{align}
y_i &\sim \operatorname{Categorical}(\theta) \\
\theta_j &= \sum_{s=1}^{5} p_s \Pr(j \mid s), \quad j = 1, \ldots, 3 \\
p &\sim \operatorname{Dirichlet}([4, 4, 4, 4, 4])
\end{align}
$$


```{r}
dat_list <- list(
  N = nrow(d),
  y = d$y,
  majority_first = d$majority_first
)

m16.2 <- cmdstan_model("mod_16.2.stan")
fit16.2 <- m16.2$sample(
  data = dat_list,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 250, iter_sampling = 300,
  show_messages = F, show_exceptions = F
)

fit16.2$summary("p")
```

```{r}
#| output: asis
#| echo: false

stan_source <- htmltools::htmlEscape(
  paste(readLines("mod_16.2.stan"), collapse = "\n")
)
cat(
  "<details>\n",
  "<summary>Stan code for model 16.2</summary>\n",
  "<pre><code>", stan_source, "</code></pre>\n",
  "</details>\n",
  sep = ""
)
```

```{r}
p_labels <- c(
  "1 Majority",
  "2 Minority",
  "3 Maverick",
  "4 Random",
  "5 Follow First"
)

p_draws <- fit16.2$draws("p", format = "matrix")
p_draws <- p_draws[, grepl("^p\\[", colnames(p_draws)), drop = FALSE]
p_summary <- data.frame(
  mean = colMeans(p_draws),
  lower = apply(p_draws, 2, quantile, probs = 0.055),
  upper = apply(p_draws, 2, quantile, probs = 0.945)
)

plot(
  p_summary$mean,
  seq_along(p_labels),
  xlim = range(c(p_summary$lower, p_summary$upper)),
  ylim = c(0.5, length(p_labels) + 0.5),
  yaxt = "n",
  xlab = "Posterior probability",
  ylab = "",
  pch = 19
)
axis(2, at = seq_along(p_labels), labels = p_labels, las = 1)
arrows(
  p_summary$lower,
  seq_along(p_labels),
  p_summary$upper,
  seq_along(p_labels),
  angle = 90,
  code = 3,
  length = 0.05
)
```


It looks like "majority" and "follow first" are both equally likely here.

## 16.3 - Ordinary differential nut cracking

Let's talk about the *Panda nut*; a large nut that chimpanzees use stones to crack open.

```{r}
data(Panda_nuts)
d <- Panda_nuts
```

We'll assume that the limit factor for opening nuts is strenght, which increases with age. For this, we need to think about the weight of the chimp. We assume some simple rate of change where the current rate of increase in mass or proportionate to the distance to its max weight:

$$
\begin{align}
\frac{dM}{dt} &= k(M_{\text{max}} - M_t) \\
M_t &= M_{\text{max}} (1 - \exp(-kt))
\end{align}
$$

The equation for $M_t$ (the solution to the differential equation) is a classical one used in biology.

Now, the quantity really of interest is strength. We'll assume that it is some multiplier on top of weight: $S_t = \beta M_t$.

To take this one step further, want to understand the rate of nutcracking. We may assume that the relationship is linear (as with the translation between strength and weight), but consider all the actions that come with strength. The relationship might actually be exponential. To model this we let the rate $\lambda$ be an exponential equation of stregth: $\lambda = \alpha S_t^\theta$, where we assume $\theta > 1$.

Now we have:

$$
\lambda = \alpha (\beta M_{\text{max}} (1 - \exp(-kt)))^\theta
$$

But we can simplify. If we rescale so that $M_{\text{max}} = 1$ and let $\phi = \alpha \beta^\theta$, then $\lambda = \phi (1 - \exp(-kt))^\theta$.

Now, for the model we will use a Poisson likelihood and multiply our rate equation by some duration $d_i$ so we can equivalize against chimps that had longer to crack:

$$
\begin{align}
n_i &\sim \text{Poisson}(\lambda_i) \\
\lambda_i &= d_i \phi (1 - \exp(-kt))^\theta \\
\phi &\sim \text{Log-Normal}(\log(1), 0.1) \\
k &\sim \text{Log-Normal}(\log(2), 0.25) \\
\theta &\sim \text{Log-Normal}(\log(5), 0.25) \\
\end{align}
$$

Let's look at the implied growth rates:

```{r}
N <- 1e4
phi <- rlnorm(N, log(1), 0.1)
k <- rlnorm(N, log(2), 0.25)
theta <- rlnorm(N, log(5), 0.25)

par(mfrow = c(1, 2))
# relative grow curve
plot(NULL, xlim = c(0, 1.5), ylim = c(0, 1), xaxt = "n", xlab = "age", ylab = "body mass")
at <- c(0, 0.25, 0.5, 0.75, 1, 1.25, 1.5)
axis(1, at = at, labels = round(at * max(Panda_nuts$age)))
for (i in 1:20) curve((1 - exp(-k[i] * x)), add = TRUE, col = grau(), lwd = 1.5)

# implied rate of nut opening curve
plot(NULL, xlim = c(0, 1.5), ylim = c(0, 1.2), xaxt = "n", xlab = "age", ylab = "nuts per second")
at <- c(0, 0.25, 0.5, 0.75, 1, 1.25, 1.5)
axis(1, at = at, labels = round(at * max(Panda_nuts$age)))
for (i in 1:20) curve(phi[i] * (1 - exp(-k[i] * x))^theta[i], add = TRUE, col = grau(), lwd = 1.5)
```

Now we can build the model:


```{r}
dat_list <- list(
  N = nrow(d),
  n = as.integer(d$nuts_opened),
  age = d$age / max(d$age),
  seconds = d$seconds
)

m16.3 <- cmdstan_model("mod_16.3.stan")
fit16.3 <- m16.3$sample(
  data = dat_list,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 250, iter_sampling = 300,
  show_messages = F, show_exceptions = F
)
```
```{r}
#| output: asis
#| echo: false

stan_source <- htmltools::htmlEscape(
  paste(readLines("mod_16.3.stan"), collapse = "\n")
)
cat(
  "<details>\n",
  "<summary>Stan code for model 16.3</summary>\n",
  "<pre><code>", stan_source, "</code></pre>\n",
  "</details>\n",
  sep = ""
)
```

Now let's look at the posterior predictive distribution.
```{r}
par(mfrow = c(1, 1))
post <- fit16.3$draws(format = "df") |> select(-starts_with("."))
plot(NULL, xlim = c(0, 1), ylim = c(0, 1.5), xlab = "age", ylab = "nuts per second", xaxt = "n")
at <- c(0, 0.25, 0.5, 0.75, 1, 1.25, 1.5)
axis(1, at = at, labels = round(at * max(d$age)))
# raw data
pts <- dat_list$n / dat_list$seconds
point_size <- normalize(dat_list$seconds)
points(jitter(dat_list$age), pts, col = rangi2, lwd = 2, cex = point_size * 3)
# 30 posterior curves
for (i in 1:30) {
  with(
    post,
    curve(phi[i] * (1 - exp(-k[i] * x))^theta[i], add = TRUE, col = grau())
  )
}
```

One thing to note is that we could have used individual specific parameters where they make sense. But $\theta$ makes sense to remain global as it is a governing property of physics and not something that can change by chimp.

## 16.4 - Population Dynamics

We'll study here the population times series for lynx and hares:


```{r}
data(Lynx_Hare)
plot(1:21, Lynx_Hare[, 3],
  ylim = c(0, 90), xlab = "year",
  ylab = "thousands of pelts", xaxt = "n", type = "l", lwd = 1.5
)
at <- c(1, 11, 21)
axis(1, at = at, labels = Lynx_Hare$Year[at])
lines(1:21, Lynx_Hare[, 2], lwd = 1.5, col = rangi2)
points(1:21, Lynx_Hare[, 3], bg = "black", col = "white", pch = 21, cex = 1.4)
points(1:21, Lynx_Hare[, 2], bg = rangi2, col = "white", pch = 21, cex = 1.4)
text(17, 80, "Lepus", pos = 2)
text(19, 50, "Lynx", pos = 2, col = rangi2)
```

This is actually a model of the number of pelts, not the actual animals.

A simple model would be an autoregressive one. Something like:

$$
E(H_t) = \alpha + \beta_1 H_{t-1}
$$

Where $H_t$ is the number of hares at time $t$.

We can do better than this though. We'll set up a simple ODE:

$$
\frac{dH}{dt} = H_t \times b_H - H_t \times m_H
$$

Where $b_H$ and $m_H$ are the birth and death rate of hares, respectively. We can also make the assumption that the number of lynxs present impact the *per capita rate* ($b_H - m_H$) via multiplication into the death rate:

$$
\frac{dH}{dt} = H_t (b_H - L_t m_H)
$$

A ODE can be made for the Lynx rate of change, except we multiply the birth rate of the lynx by the current hare population. Let's go ahead a simulate this model:


```{r}
sim_lynx_hare <- function(n_steps, init, theta, dt = 0.002) {
  L <- rep(NA, n_steps)
  H <- rep(NA, n_steps)
  L[1] <- init[1]
  H[1] <- init[2]
  for (i in 2:n_steps) {
    H[i] <- H[i - 1] + dt * H[i - 1] * (theta[1] - theta[2] * L[i - 1])
    L[i] <- L[i - 1] + dt * L[i - 1] * (theta[3] * H[i - 1] - theta[4])
  }
  return(cbind(L, H))
}

theta <- c(0.5, 0.05, 0.025, 0.5)
z <- sim_lynx_hare(1e4, as.numeric(Lynx_Hare[1, 2:3]), theta)

plot(z[, 2],
  type = "l", ylim = c(0, max(z[, 2])), lwd = 2, xaxt = "n",
  ylab = "number (thousands)", xlab = ""
)
lines(z[, 1], col = rangi2, lwd = 2)
mtext("time", 1)
```

One concept we need to introduce is the latent and observeed state. We'll say that $h_t$ is what we actualy observe from the pelts, but $H_t$ is the latent state for the actual hare population. We'll also say that there is some trap rate $p$ that is applied to the hare population. Let's use a $\text{Beta}(2,18)$ distribution for $p$ so that we are centered around 10% trapping rate. Let's simulate to see if it makes sense:


```{r}
N <- 1e4
Ht <- 1e4
p <- rbeta(N, 2, 18)
h <- rbinom(N, size = Ht, prob = p)
h <- round(h / 1000, 2)
dens(h, xlab = "thousand of pelts", lwd = 2)
```

Looks reasonable, with a long tail. We'll use a lognormal distribution to model the observed pelts:

$$
h_t \sim \text{Log-Normal}(\log(p H_t), \sigma_H)
$$

This sets our median at $p H_t$. Note that $p$ is very hard to identify here. So instead we will just use a strong prior. Now we're ready to write the model:

$$
\begin{align}
h_t &\sim \operatorname{LogNormal}\left(\log(p_H H_t), \sigma_H\right) \\
\ell_t &\sim \operatorname{LogNormal}\left(\log(p_L L_t), \sigma_L\right)
\end{align}
$$

Now we need to model the actual unobserved states:

$$
\begin{align}
H_1 &\sim \operatorname{LogNormal}(\log 10, 1) \\
L_1 &\sim \operatorname{LogNormal}(\log 10, 1) \\
H_{T>1} &= H_1 + \int_1^T H_t \left(b_H - m_H L_t\right)\,dt \\
L_{T>1} &= L_1 + \int_1^T L_t \left(b_L H_t - m_L\right)\,dt
\end{align}
$$

Note that we set a prior on the inital state. The integration is how we back out the rate formula shown before. Finally, we have our parameter priors:

$$
\begin{align}
\sigma_H &\sim \operatorname{Exponential}(1) \\
\sigma_L &\sim \operatorname{Exponential}(1) \\
p_H &\sim \operatorname{Beta}(\alpha_H, \beta_H) \\
p_L &\sim \operatorname{Beta}(\alpha_L, \beta_L) \\
b_H &\sim \operatorname{HalfNormal}(1, 0.5) \\
b_L &\sim \operatorname{HalfNormal}(0.05, 0.05) \\
m_H &\sim \operatorname{HalfNormal}(0.05, 0.05) \\
m_L &\sim \operatorname{HalfNormal}(1, 0.5)
\end{align}
$$


Now we can fit our model!

```{r}
dat_list <- list(
  N = nrow(Lynx_Hare),
  pelts = Lynx_Hare[, 2:3]
)

m16.4 <- cmdstan_model("mod_16.4.stan")
fit16.4 <- m16.4$sample(
  data = dat_list,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 800, iter_sampling = 800, adapt_delta = 0.99,
  show_messages = F, show_exceptions = F
)
```
```{r}
#| output: asis
#| echo: false

stan_source <- htmltools::htmlEscape(
  paste(readLines("mod_16.4.stan"), collapse = "\n")
)
cat(
  "<details>\n",
  "<summary>Stan code for model 16.4</summary>\n",
  "<pre><code>", stan_source, "</code></pre>\n",
  "</details>\n",
  sep = ""
)
```
Now let's look at the posterior predictive:

```{r}
hare <- fit16.4$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("2]"))
lynx <- fit16.4$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("1]"))
pelts <- dat_list$pelts

plot(1:21, pelts[, 2],
  pch = 16, ylim = c(0, 120), xlab = "year",
  ylab = "thousandsof pelts", xaxt = "n"
)
at <- c(1, 11, 21)
axis(1, at = at, labels = Lynx_Hare$Year[at])
points(1:21, pelts[, 1], col = rangi2, pch = 16)
# 21 timeseries from posterior

for (s in 1:21) {
  lines(1:21, hare[s, ], col = col.alpha("black", 0.2), lwd = 2)
  lines(1:21, lynx[s, ], col = col.alpha(rangi2, 0.3), lwd = 2)
}
# text labels
text(17, 90, "Lepus", pos = 2)
text(19, 50, "Lynx", pos = 2, col = rangi2)
```

We are see jagged lines since the *measurement error* ($\sigma_H$) is uncorrelated across time. We can also plot the latent trend too:


```{r}
hare_pop <- fit16.4$draws(format = "df") |>
  select(starts_with("pop[")) |>
  select(ends_with("2]"))
lynx_pop <- fit16.4$draws(format = "df") |>
  select(starts_with("pop[")) |>
  select(ends_with("1]"))

plot(NULL,
  pch = 16, xlim = c(1, 21), ylim = range(hare_pop[1:21, ], lynx_pop[1:21, ]), xlab = "year",
  ylab = "thousandsofanimals", xaxt = "n"
)
at <- c(1, 11, 21)

axis(1, at = at, labels = Lynx_Hare$Year[at])
for (s in 1:21) {
  lines(1:21, hare_pop[s, ], col = col.alpha("black", 0.2), lwd = 2)
  lines(1:21, lynx_pop[s, ], col = col.alpha(rangi2, 0.4), lwd = 2)
}
```

## Exercises

### M.1

We will edit the cylinder height model to instead have a free parameter on the exponent (what was 3 in the problem):


```{r}
data(Howell1)
d <- Howell1
d$w <- d$weight / mean(d$weight)
d$h <- d$height / mean(d$height)

stan_data <- list(
  N = nrow(d),
  w = d$w,
  h = d$h
)

m16.5 <- cmdstan_model("mod_16.5.stan")
fit16.5 <- m16.5$sample(
  data = stan_data,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 250, iter_sampling = 300,
  # max_treedepth = 15,
  show_messages = F, show_exceptions = F
)
```

```{r}
#| output: asis
#| echo: false

stan_source <- htmltools::htmlEscape(
  paste(readLines("mod_16.5.stan"), collapse = "\n")
)
cat(
  "<details>\n",
  "<summary>Stan code for model 16.5</summary>\n",
  "<pre><code>", stan_source, "</code></pre>\n",
  "</details>\n",
  sep = ""
)
```


Let's take a look at the posterior for our exponent. You can see that it is clearly not 3:
```{r}
fit16.5$draws("alpha") |> bayesplot::mcmc_hist("alpha")
```

Let's plot the posterior predictive. Notice that when the exponent is allowed to vary, we see a much better fit at the extremes of the data.
```{r}
#| echo: false
#| fig-width: 12
#| fig-height: 7
#| fig-align: center
par(mfrow = c(1, 2))

h_seq <- seq(from = 0, to = max(d$h), length.out = 30)
posterior <- fit16.1$draws(c("p", "k", "sigma"), format = "df")
w_sim <- vapply(
  h_seq,
  function(h) {
    rlnorm(
      nrow(posterior),
      meanlog = log(3.141593 * posterior$k * posterior$p^2 * h^3),
      sdlog = posterior$sigma
    )
  },
  numeric(nrow(posterior))
)
mu_mean <- apply(w_sim, 2, mean)
w_CI <- apply(w_sim, 2, PI)

plot(
  d$h,
  d$w,
  xlim = c(0, max(d$h)),
  col = rangi2,
  lwd = 2,
  xlab = "height (scaled)",
  ylab = "weight (scaled)",
  main = "Original Model (m.1)"
)
lines(h_seq, mu_mean)
shade(w_CI, h_seq)

h_seq <- seq(from = 0, to = max(d$h), length.out = 30)
posterior <- fit16.5$draws(c("p", "k", "sigma", "alpha"), format = "df")
w_sim <- vapply(
  h_seq,
  function(h) {
    rlnorm(
      nrow(posterior),
      meanlog = log(3.141593 * posterior$k * posterior$p^2 * h^posterior$alpha),
      sdlog = posterior$sigma
    )
  },
  numeric(nrow(posterior))
)
mu_mean <- apply(w_sim, 2, mean)
w_CI <- apply(w_sim, 2, PI)

plot(
  d$h,
  d$w,
  xlim = c(0, max(d$h)),
  col = rangi2,
  lwd = 2,
  xlab = "height (scaled)",
  ylab = "weight (scaled)",
  main = "Model where exponent can vary"
)
lines(h_seq, mu_mean)
shade(w_CI, h_seq)
```

### M.3

Let's perform some prior preditive simulations from the lynx-hare model:

```{r}
N <- 20
m16.4_prior_pred <- cmdstan_model("mod_16.4_prior_pred.stan")
pp_m16.4 <- m16.4_prior_pred$sample(
  data = list(N = N),
  chains = 1,
  iter_warmup = 200, iter_sampling = 300,
  show_messages = F, show_exceptions = F
)
```

```{r}
#| output: asis
#| echo: false

stan_source <- htmltools::htmlEscape(
  paste(readLines("mod_16.4_prior_pred.stan"), collapse = "\n")
)
cat(
  "<details>\n",
  "<summary>Stan code for Prior Predictive Checks on mod_16.4</summary>\n",
  "<pre><code>", stan_source, "</code></pre>\n",
  "</details>\n",
  sep = ""
)
```

```{r}
par(mfrow = c(1, 2))
hare_prior <- pp_m16.4$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("2]"))
lynx_prior <- pp_m16.4$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("1]"))

na_filt <- (hare_prior |> is.na() |> apply(1, max) |> as.logical()) | (lynx_prior |> is.na() |> apply(1, max) |> as.logical())
hare_prior <- hare_prior[!na_filt, ]
lynx_prior <- lynx_prior[!na_filt, ]


plot(NULL,
  pch = 16, xlim = c(1, 21), ylim = range(hare_prior, lynx_prior), xlab = "year",
  ylab = "thousandsofanimals", xaxt = "n"
)
at <- c(1, 11, 21)

axis(1, at = at, labels = Lynx_Hare$Year[at])

for (s in 1:min(nrow(hare_prior), 20)) {
  lines(1:N, hare_prior[s, ], col = col.alpha("black", 0.2), lwd = 2)
  lines(1:N, lynx_prior[s, ], col = col.alpha(rangi2, 0.4), lwd = 2)
}


plot(NULL,
  pch = 16, xlim = c(1, 21), ylim = c(0, 120), xlab = "year",
  ylab = "thousandsofanimals", xaxt = "n"
)
at <- c(1, 11, 21)

axis(1, at = at, labels = Lynx_Hare$Year[at])

for (s in 1:min(nrow(hare_prior), 20)) {
  lines(1:N, hare_prior[s, ], col = col.alpha("black", 0.2), lwd = 2)
  lines(1:N, lynx_prior[s, ], col = col.alpha(rangi2, 0.4), lwd = 2)
}
```

Looks like they are not following the previous enough. It needs to be more steady. One way to induce this would be to impose some sort of negative correlation between the parameters. Take a look at the learned parameters for `mod16.4` for example:

```{r}
fit16.4$draws(format = "df") |>
  select("m_H", "b_L", "m_L", "b_H") |>
  cor()
```

Let's also look at the difference between the fitted model and the prior pred:

```{r}
params <- c("m_H", "b_L", "m_L", "b_H", "pop_init[1]", "pop_init[2]")

fit_draws <- fit16.4$draws(params, format = "df")
prior_draws <- pp_m16.4$draws(params, format = "df")

par(mfrow = c(2, 3), mar = c(4, 4, 3, 1))

for (param in params) {
  fit_values <- fit_draws[[param]]
  prior_values <- prior_draws[[param]]

  summaries <- rbind(
    posterior = c(
      lower = min(fit_values),
      mean = mean(fit_values),
      upper = max(fit_values)
    ),
    prior_predictive = c(
      lower = min(prior_values),
      mean = mean(prior_values),
      upper = max(prior_values)
    )
  )

  plot(
    NA,
    xlim = c(0.5, 2.5),
    ylim = range(summaries[, c("lower", "upper")]),
    xaxt = "n",
    xlab = "",
    ylab = "Parameter value",
    main = param
  )
  axis(1, at = 1:2, labels = c("Posterior", "Prior predictive"))

  segments(
    x0 = 1:2,
    y0 = summaries[, "lower"],
    x1 = 1:2,
    y1 = summaries[, "upper"],
    lwd = 3,
    col = c("firebrick", "steelblue")
  )
  points(
    1:2,
    summaries[, "mean"],
    pch = 19,
    cex = 1.4,
    col = c("firebrick", "steelblue")
  )
}
```

Let's first start with making the $m_H$ and $b_L$ priors much smaller and see if that improves things:

```{r}
N <- 20
m16.4_prior_pred_2 <- cmdstan_model("mod_16.4_prior_pred_2.stan")
pp_m16.4_2 <- m16.4_prior_pred_2$sample(
  data = list(N = N),
  chains = 1,
  iter_warmup = 200, iter_sampling = 300,
  show_messages = F, show_exceptions = F
)
```


```{r}
par(mfrow = c(1, 1))
hare_prior <- pp_m16.4_2$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("2]"))
lynx_prior <- pp_m16.4_2$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("1]"))

na_filt <- (hare_prior |> is.na() |> apply(1, max) |> as.logical()) | (lynx_prior |> is.na() |> apply(1, max) |> as.logical())
hare_prior <- hare_prior[!na_filt, ]
lynx_prior <- lynx_prior[!na_filt, ]


plot(NULL,
  pch = 16, xlim = c(1, 21), ylim = range(hare_prior, lynx_prior), xlab = "year",
  ylab = "thousandsofanimals", xaxt = "n"
)
at <- c(1, 11, 21)

axis(1, at = at, labels = Lynx_Hare$Year[at])

for (s in 1:min(nrow(hare_prior), 20)) {
  lines(1:N, hare_prior[s, ], col = col.alpha("black", 0.2), lwd = 2)
  lines(1:N, lynx_prior[s, ], col = col.alpha(rangi2, 0.4), lwd = 2)
}
```

Didn't really help much even though the parameters seem to be better controlled:
```{r}
params <- c("m_H", "b_L", "m_L", "b_H", "pop_init[1]", "pop_init[2]")

fit_draws <- fit16.4$draws(params, format = "df")
prior_draws <- pp_m16.4_2$draws(params, format = "df")

par(mfrow = c(2, 3), mar = c(4, 4, 3, 1))

for (param in params) {
  fit_values <- fit_draws[[param]]
  prior_values <- prior_draws[[param]]

  summaries <- rbind(
    posterior = c(
      lower = min(fit_values),
      mean = mean(fit_values),
      upper = max(fit_values)
    ),
    prior_predictive = c(
      lower = min(prior_values),
      mean = mean(prior_values),
      upper = max(prior_values)
    )
  )

  plot(
    NA,
    xlim = c(0.5, 2.5),
    ylim = range(summaries[, c("lower", "upper")]),
    xaxt = "n",
    xlab = "",
    ylab = "Parameter value",
    main = param
  )
  axis(1, at = 1:2, labels = c("Posterior", "Prior predictive"))

  segments(
    x0 = 1:2,
    y0 = summaries[, "lower"],
    x1 = 1:2,
    y1 = summaries[, "upper"],
    lwd = 3,
    col = c("firebrick", "steelblue")
  )
  points(
    1:2,
    summaries[, "mean"],
    pch = 19,
    cex = 1.4,
    col = c("firebrick", "steelblue")
  )
}
```

If we really want to get this right, we need to implement correlations between the rate parameters.

### H2

Returning to the Panda Nut problem - let's now look at things as they vary by Chimpanzee. We'll choose to vary $\phi$ by Chimpanzee here.

```{r}
data(Panda_nuts)
d <- Panda_nuts

dat_list <- list(
  N = nrow(d),
  n = as.integer(d$nuts_opened),
  age = d$age / max(d$age),
  seconds = d$seconds,
  chimp = as.integer(d$chimpanzee)
)

mh2 <- cmdstan_model("mod_h2.stan")
fit_h2 <- mh2$sample(
  data = dat_list,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 250, iter_sampling = 300,
  show_messages = F, show_exceptions = F
)
```

```{r}
#| output: asis
#| echo: false

stan_source <- htmltools::htmlEscape(
  paste(readLines("mod_h2.stan"), collapse = "\n")
)
cat(
  "<details>\n",
  "<summary>Stan code for mod_h2</summary>\n",
  "<pre><code>", stan_source, "</code></pre>\n",
  "</details>\n",
  sep = ""
)
```


Notice that in the stan code we had to exponentiate both the previous prior for $\phi$, now the mean of the population distribution, as well as the reconstructed $\phi$ vector in the new model. This is because we wanted to model with a normal population, but that would give us negative values. Thus, we just exponentiate our problems away.

### H3

Let's turn back to the Lynx-Hare model. Previously we used an ODE to model the dynamics. Now we'll turn to pure autoregressive with the equations looking like:

$$
\begin{align}
L_t &\sim \operatorname{LogNormal}(\log \mu_{L,t}, \sigma_L) \\
\mu_{L,t} &= \alpha_L + \beta_{LL} L_{t-1} + \beta_{LH} H_{t-1} \\
H_t &\sim \operatorname{LogNormal}(\log \mu_{H,t}, \sigma_H) \\
\mu_{H,t} &= \alpha_H + \beta_{HH} H_{t-1} + \beta_{HL} L_{t-1}
\end{align}
$$



```{r}
dat_list <- list(
  N = nrow(Lynx_Hare),
  pelts = Lynx_Hare[, 2:3]
)

mod_h3 <- cmdstan_model("mod_h3.stan")
fit_h3 <- mod_h3$sample(
  data = dat_list,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 600, iter_sampling = 300,
  adapt_delta = 0.99,
  show_messages = F, show_exceptions = F
)
```


```{r}
#| fig-width: 12
#| fig-height: 12
#| fig-align: center
par(mfrow = c(2, 1))
hare <- fit16.4$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("2]"))
lynx <- fit16.4$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("1]"))
pelts <- dat_list$pelts

plot(1:21, pelts[, 2],
  pch = 16, ylim = c(0, 120), xlab = "year",
  ylab = "thousandsof pelts", xaxt = "n"
)
at <- c(1, 11, 21)
axis(1, at = at, labels = Lynx_Hare$Year[at])
points(1:21, pelts[, 1], col = rangi2, pch = 16)
# 21 timeseries from posterior

for (s in 1:21) {
  lines(1:21, hare[s, ], col = col.alpha("black", 0.2), lwd = 2)
  lines(1:21, lynx[s, ], col = col.alpha(rangi2, 0.3), lwd = 2)
}
# text labels
text(17, 90, "Lepus", pos = 2)
text(19, 50, "Lynx", pos = 2, col = rangi2)

hare <- fit_h3$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("2]"))
lynx <- fit_h3$draws(format = "df") |>
  select(starts_with("pelts_pred")) |>
  select(ends_with("1]"))
pelts <- dat_list$pelts

plot(1:21, pelts[, 2],
  pch = 16, ylim = c(0, 120), xlab = "year",
  ylab = "thousandsof pelts", xaxt = "n"
)
at <- c(1, 11, 21)
axis(1, at = at, labels = Lynx_Hare$Year[at])
points(1:21, pelts[, 1], col = rangi2, pch = 16)
# 21 timeseries from posterior

for (s in 1:21) {
  lines(1:21, hare[s, ], col = col.alpha("black", 0.2), lwd = 2)
  lines(1:21, lynx[s, ], col = col.alpha(rangi2, 0.3), lwd = 2)
}
# text labels
text(17, 90, "Lepus", pos = 2)
text(19, 50, "Lynx", pos = 2, col = rangi2)
```

It is clear to see that the ODE produces a much cleaner, smoother fit.